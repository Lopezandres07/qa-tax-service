...
When('el usuario navega hasta la sección de planes', async function () {
  const landing = new LandingPage(this.page)
  await landing.seccionPlanes.scrollIntoViewIfNeeded()
})

Then('se deben visualizar los precios de los servicios', async function () {
  const landing = new LandingPage(this.page)
...
Then('el usuario es redirigido a la página de pago', async function () {
  await expect(this.page).toHaveURL(/.*checkout/)
})

When('el usuario visualiza un plan sin precio definido', async function () {
  const landing = new LandingPage(this.page)
  await expect(landing.obtenerPrecioPlan('')).toHaveText('')
})

When('intenta hacer click en "Start Now"', async function () {
  const landing = new LandingPage(this.page)
  await landing.clickEnStartNow('')
})

Then('el sistema debe bloquear la redirección al checkout', async function () {
  await expect(this.page).not.toHaveURL(/.*checkout/)
})

Then('debe mostrar un aviso de {string}', async function (mensaje) {
  const landing = new LandingPage(this.page)
